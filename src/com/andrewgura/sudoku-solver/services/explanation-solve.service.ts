...
import EntanglementModel from '../models/entanglement.model';
...
export default class ExplanationSolveService {
    ...
    public static solve(sudoku: Sudoku): SolveActionModel[] {
        ...
        for (let i: number = 0; i < 9; i++) {
            for (let j: number = 0; j < 9; j++) {
                if (sudoku.cells[i][j].value == 0 || sudoku.cells[i][j].status != SudokuCellStatus.Set) {
                    complexity *= sudoku.cells[i][j].possibleValuesHash.toArray().length;
                }
            }
        }
        ...
        while (atLeastOneEmpty && atLeastOneFoundOnLastIteration) {
            ...
            // find pairs, triples etc. and cleanup possible values a bit
            for (const cellSet of sudoku.cellSets) {
                const entanglements: EntanglementModel[] = cellSet.findEntanglements();
                for (const entanglement of entanglements) {
                    for (const cell of cellSet.cells) {
                        if (entanglement.cells.includes(cell)) {
                            cell.possibleValuesHash.fromArray(entanglement.values);
                        } else {
                            for (const value of entanglement.values) {
                                cell.possibleValuesHash.removeItem(value);
                            }
                        }
                    }
                }
            ...
            for (let i: number = 0; i < 9; i++) {
                for (let j: number = 0; j < 9; j++) {
                    ...
                    if (cell.possibleValuesHash.isSingleElement()) {
                        const value: number = cell.possibleValuesHash.toArray()[0];
